...
export function ChatsTab({ user }: ChatsTabProps) {
  ...
  const [miniProfile, setMiniProfile] = useState<MiniProfileTarget | null>(null);
  ...
  const handleAuthorClick = (authorUserId: number | null | undefined, authorName: string) => {
    if (authorUserId && authorUserId > 0) {
      setMiniProfile({ userId: authorUserId, fallbackName: authorName });
    }
  };
  ...
  if (activeRoom) {
    ...
    return (
      <>
        {miniProfile && (
          <UserMiniProfile userId={miniProfile.userId} fallbackName={miniProfile.fallbackName} onClose={() => setMiniProfile(null)} />
        )}
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: isMe ? "flex-end" : "flex-start", maxWidth: "80%" }}>
                    {!isMe && (
                      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                        <button
                          onClick={() => handleAuthorClick(msg.author_user_id, msg.author)}
                          style={{ border: 0, background: "transparent", padding: 0, cursor: msg.author_user_id ? "pointer" : "default", fontSize: 11, color: "var(--accent)", fontWeight: 700 }}
                        >
                          {msg.author}
                        </button>
                      </div>
